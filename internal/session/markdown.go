...
// ExportMeta is header info for a markdown export.
...
// RenderMarkdown turns conversation entries into a readable Markdown document.
func RenderMarkdown(meta ExportMeta, entries []ctxmgr.ExportedEntry) string {
...
// WriteExportFile writes markdown to path, creating parent directories.
...
func writeExportEntry(b *strings.Builder, e ctxmgr.ExportedEntry) {
	...
	case llm.RoleTool:
		...
		b.WriteString("\n\n```\n")
		body := msg.Content
		body = strutil.TruncateRunes(body, exportToolResultMax)
		b.WriteString(body)
